...
export const erpSlice = createSlice({  name: "erp", initialState,
  reducers: { resetState(state, action) { state = initialState},
    ...
    // requestLoading(state, action) { state = { ...state, [action.payload.keyState]: { ...state[action.payload.keyState], isLoading: true, }, } },
    // requestFailed(state, action) { state = { ...state, [action.payload.keyState]: { ...state[action.payload.keyState], isLoading: false, isSuccess: false, }, } },
    ...
    requestSuccess(state, action) { console.log('18JUN23-10:10:17 AM', JSON.stringify(action));
      ...
      // result: action.payload.result, isLoading: false, isSuccess: true, current: null }, };
      state[action.payload.keyState].result = action.payload.result;
      state[action.payload.keyState].isLoading = false;
      state[action.payload.keyState].isSuccess = true;
      state[action.payload.keyState].current = null;
...
export const { resetState, currentItem, requestLoading, requestFailed, requestSuccess, currentAction, resetAction, setTotals, delItemFrmState, editItemInState, } = erpSlice.actions;
