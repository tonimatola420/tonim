...
import { getSearchedItems, getList, delItemById, editItemById, findItemById } from '@/utils/client/api/ledgerApis';
...
export const erpSlice = createSlice({
  name: "erp", initialState,
  reducers: {
    resetState(state, action) { return initialState; },
    currentItem(state, action) { return { ...state, current: { result: action.payload, }, }; },
    requestLoading(state, action) { return { ...state, [action.payload.keyState]: { ...state[action.payload.keyState], isLoading: true, }, } },
    requestFailed(state, action) { return { ...state, [action.payload.keyState]: { ...state[action.payload.keyState], isLoading: false, isSuccess: false, }, } },
    requestSuccess(state, action) { 
      // console.log('18JUN23-10:10:110 AM', action.payload.keyState, ' == ' ,JSON.stringify(state.list.isLoading));
      return { ...state, [action.payload.keyState]: { ...state[action.payload.keyState], result: action.payload.result, isLoading: false, isSuccess: true, current: null, }, };
    },
    currentAction(state, action) {
      return {
        ...state, [action.payload.keyState]: {
          ...state[action.payload.keyState],
          current: state.list.result.items.find((item: any) => item.id === action.payload.id), isSuccess: false,
        },
      };
    ...
    setHdrDtlUpdtItem(state, action) {
      return {
        ...state, [action.payload.keyState]: { ...state[action.payload.keyState], current: action.payload.result, isLoading: false, isSuccess: false, },
      };
    },
    resetAction(state, action) {
      return { ...state, [action.payload.keyState]: initialState[action.payload.keyState], };
...
export const { resetState, currentItem, requestLoading, requestFailed, requestSuccess, currentAction, resetAction, delItemFrmState, editItemInState, setHdrDtlUpdtItem, } = erpSlice.actions;
export const selectUpdateState = (state: AppState) => state.erp.update; export const selectDeleteState = (state: AppState) => state.erp.delete;
export const selectCreateState = (state: AppState) => state.erp.create; export const selectListState = (state: AppState) => state.erp.list;
...
export function fetchList(entity: string, options: IOptions = {}) {
  return async function fetchListThunk(dispatch: any, getState: any) {
    ...
    try {
      const data: any = await getList(entity, options); // console.log('somak 13JUN 09:25:52 PM = ', JSON.stringify(ldata));
      if (data.success) {
        const result = {
...
export function fetchById(entity: string, options: IOptions = {}) {
  return async function fetchListThunk(dispatch: any, getState: any) {
    dispatch(requestLoading({ keyState: 'update' }));
    try {
      const data: any = await findItemById(entity, options); // console.log('somak 13JUN 09:25:52 PM = ', JSON.stringify(ldata));
      if (data.success) {
        const result = data.result;
        console.log('18JUN2023-10:03:56 AM', JSON.stringify(result));
        dispatch(setHdrDtlUpdtItem({ keyState: 'update', result: result }));
      }
    } catch (err) { console.log(err); dispatch(requestFailed({ keyState: 'list' })); }
  };
}
