...
export default function DataTable({ DropDownRowMenu, AddNewItem }:
    ...
    const handelDataTableLoad = useCallback((pagination: any) => {
        const options = { page: pagination.current || '1', relations: JSON.stringify({ sector: true, }), jsonData: '{}' };
        dispatch(fetchList(entity, options));
    }, []);

    useEffect(() => {
        const options = { page: 1, relations: JSON.stringify({ sector: true, }), jsonData: '{}' };
        dispatch(fetchList(entity, options));        
    }, []);
