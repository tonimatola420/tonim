...
export default function UpdateForm() {
  ...
  //   (value: dayjs.Dayjs | null, dateString: string): void => { setFrmdate(dateString); };
  const handleJrdateChange = (value: dayjs.Dayjs | null, dateString: string): void => {
    setFrmdate(dayjs(value).subtract(0, 'days').format('DD-MM-YYYY'));
    // setFrmdate(dateString); 
    // console.log('30jun23-09:41: ', value, ' == ', dateString);
  };
  const cancelEdit = () => {
  ...
  useEffect(() => {
    if (current) {
      ...
      setFrmdate(dayjs(newValues['date'].$d).subtract(0, 'days').format('DD-MM-YYYY'));      
      form.setFieldsValue(newValues);
      // console.log('🚀 ~ file: index.jsx ~ line 40 ~ useEffect ~ obj', frmdate, 
      //   newValues['date'].$d, ' == ', dayjs(newValues['date'].$d).subtract(0, 'days').format('DD-MM-YYYY'));
    }
  }, [current]);

  return (
    <ERPLayout title="Transaction New" frmwidth="100">
