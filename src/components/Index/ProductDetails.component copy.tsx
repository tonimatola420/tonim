...
const ProductDetails: React.FC<{ product: IProductObject }> = ({ product }) => {
  ...
  const { data } = useAppGlobalState();
  // let ctxproducts: any[]  = [];
  // let prdpos: number=-1;
  // const ctxproducts  = data.map((item) => {
  //   if(item.name == 'myproducts'){
  //     return item.value;
  //   }
  //   else return null;
  // }).filter(i => i);

  // // console.log('somaksuruuu', JSON.stringify(ctxproducts![0]));
  // // if(ctxproducts[0]){
  //   for (let n = 0; n < ctxproducts[0]!.length; n++) {
  //     console.log('somak rraaaa', ctxproducts[0]![n]._id);
  //     console.log('somak rraaaa', product._id);
  //     if (ctxproducts[0]![n]._id == product._id) {
  //       console.log('somak rraaaa', ctxproducts[0]![n]._id);
  //       prdpos = n;
  //       break;
  //     }
  ...
  //   const rdata = await userPostProductReview(product._id, { rating, comment });
  //   // console.log(userInfo?.token);
  //   // console.log(JSON.parse(userInfo!.toString()));

  //   // try {
  //   //   // const response = 
  //   //   await fetch(`http://127.0.0.1:5000/api/products/${product._id}/reviews`, {
  //   //     headers: {
    ...
    fetch(url, {
      ...
      .then((response) => {
        ...
        if (response.ok) {
          ...
          // });
          // setReviews([...reviews, {name: 'a', comment: 'a', updatedAt: 'a'},{name: 'z', comment: 'z', updatedAt: 'z'}]);
          setReviews(reviews => [...reviews, {
            name: userInfo!.name, comment, updatedAt: Date().toLocaleString(),
          }]);
          // ctxproducts[0]![prdpos].reviews = reviews;
          // dispatch("add", { name: 'myproducts', value: ctxproducts[0], lenid: 4 });
  ...
  const ulogin = async () => {
    ...
    // console.log('somakisalalalaaa....', JSON.stringify(data));
    // console.log('somakisalalalaaa....', JSON.parse(localStorage.getItem('userInfo')!));
    setLoginVisible(false);
    // setLoggedIn(!loggedIn);
    // return data;
    setUserInfo(JSON.parse(localStorage.getItem('userInfo')!));
  }
  // if (typeof window !== 'undefined' && typeof localStorage.getItem('userInfo') !== 'undefined') {
  ...
  useEffect(() => {   
    ...
    // Perform localStorage action
    setUserInfo(JSON.parse(localStorage.getItem('userInfo')!));
    // console.log('somak rvoke', userInfo);
  }, [])
